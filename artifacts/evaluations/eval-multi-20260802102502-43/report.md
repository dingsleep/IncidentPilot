# Evaluation eval-multi-20260802102502-43

Suite: `train-v2-score-v5`  
Candidate: `p1-f871693e17e3:qwen3.7-flash:json_output:q-2f310decbea1:t-telemetry-v9:s-v8-tax4`  
Mode: `multi`

| Score | Root cause | Evidence | Hard failures | Cost (micro-USD) | Duration (ms) |
|---:|---:|---:|---:|---:|---:|
| 0.887 | 0.750 | 1.000 | 0 | 3209 | 98706 |

## Failed Episodes

- [product-catalog-failure-001](report.json#product-catalog-failure-001) — 0.550; score below 1.0
